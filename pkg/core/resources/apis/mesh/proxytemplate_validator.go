...
func validateVirtualHostModification(vHostMod *mesh_proto.ProxyTemplate_Modifications_VirtualHost) validators.ValidationError {
	...
	case mesh_proto.OpAdd:
		...
		if err := ValidateAnyResourceYAML(vHostMod.Value, &envoy_route_v3.VirtualHost{}); err != nil {
			verr.AddViolation("value", "native Envoy resource is not valid: "+err.Error())
		}
...
func validateImports(imports []string, availableProfiles map[string]struct{}) validators.ValidationError {
	...
	for i, imp := range imports {
		...
		if _, ok := availableProfiles[imp]; !ok {
			...
			verr.AddViolationAt(validators.RootedAt("imports").Index(i), "profile not found. Available profiles: "+strings.Join(profiles, ","))
		}
	}
	return verr
}
...
func validateResources(resources []*mesh_proto.ProxyTemplateRawResource) validators.ValidationError {
	...
	for i, resource := range resources {
		...
		if resource.Resource == "" {
			...
			verr.AddViolationAt(validators.RootedAt("resources").Index(i).Field("resource"), "native Envoy resource is not valid: "+err.Error())
		}
	}
	return verr
}
